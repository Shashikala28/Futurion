...
// --- Helper: Retry logic for temporary Groq overloads ---
async function generateWithRetry(prompt, retries = 3, delay = 3000) {
  for (let i = 0; i < retries; i++) {
    try {
      ...
      // Retry if Groq is overloaded (503 or 429)
      if (
        (error.message.includes("503") || error.message.includes("429")) &&
        i < retries - 1
      ) {
        console.warn(
          `⚠️ Groq temporarily unavailable, retrying in ${delay / 1000}s...`
        );
        await new Promise((res) => setTimeout(res, delay));
      } else {
        throw error;
...
export const generateAIInsights = async (industry) => {
  ...
  try {
    ...
    try {
      ...
      console.error("❌ Failed to parse Groq JSON:", cleanedText);
      throw new Error("Invalid JSON format from Groq API");
    }
  } catch (error) {
    console.warn("⚠️ Groq API failed. Using fallback data.");

    // Fallback: Provide mock data when API is unavailable
    return {
      salaryRanges: [
        {
